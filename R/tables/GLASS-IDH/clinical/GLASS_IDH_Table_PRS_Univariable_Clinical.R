#################################################################################################################
# Content: Univariable post-recurrence survival Cox PH models of clinical variables
...
# summary(coxph(Surv(os_time, os_status) ~ surgery, data = all.astros ), conf.int=0.9) ## only resections
...
summary(coxph(Surv(os_time, os_status) ~ Hypermutant, data = all.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ Hypermutant, data = all.astros))
survfit(Surv(os_time, os_status) ~ Hypermutant, data = all.astros )

# Hypermutant tumors
hm.astros <- all.astros %>% 
  filter(HM_group == "HM")

summary(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = hm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = hm.astros))
survfit(Surv(os_time, os_status) ~ age_at_diagnosis, data = hm.astros )

summary(coxph(Surv(os_time, os_status) ~ sex, data = hm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ sex, data = hm.astros))
survfit(Surv(os_time, os_status) ~ sex, data = hm.astros )

# summary(coxph(Surv(os_time, os_status) ~ surgery, data = hm.astros ), conf.int=0.9) ## only resections

summary(coxph(Surv(os_time, os_status) ~ radiotherapy, data = hm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ radiotherapy, data = hm.astros))
survfit(Surv(os_time, os_status) ~ radiotherapy, data = hm.astros )

summary(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = hm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = hm.astros))
survfit(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = hm.astros )

summary(coxph(Surv(os_time, os_status) ~ grade, data = hm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ grade, data = hm.astros))
survfit(Surv(os_time, os_status) ~ grade, data = hm.astros )

# Non-hypermutant tumors
nhm.astros <- all.astros %>% 
  filter(HM_group == "NHM")

summary(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = nhm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = nhm.astros))
survfit(Surv(os_time, os_status) ~ age_at_diagnosis, data = nhm.astros )

summary(coxph(Surv(os_time, os_status) ~ sex, data = nhm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ sex, data = nhm.astros))
survfit(Surv(os_time, os_status) ~ sex, data = nhm.astros )

# summary(coxph(Surv(os_time, os_status) ~ surgery, data = nhm.astros ), conf.int=0.9) ## only resections

summary(coxph(Surv(os_time, os_status) ~ radiotherapy, data = nhm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ radiotherapy, data = nhm.astros))
survfit(Surv(os_time, os_status) ~ radiotherapy, data = nhm.astros )

summary(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = nhm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = nhm.astros))
survfit(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = nhm.astros )

summary(coxph(Surv(os_time, os_status) ~ grade, data = nhm.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ grade, data = nhm.astros))
survfit(Surv(os_time, os_status) ~ grade, data = nhm.astros )
...
summary(coxph(Surv(os_time, os_status) ~ Hypermutant, data = all.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ Hypermutant, data = all.oligos))
survfit(Surv(os_time, os_status) ~ Hypermutant, data = all.oligos )

# Hypermutant tumors
hm.oligos <- all.oligos %>% 
  filter(HM_group == "HM")

summary(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = hm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = hm.oligos))
survfit(Surv(os_time, os_status) ~ age_at_diagnosis, data = hm.oligos )

summary(coxph(Surv(os_time, os_status) ~ sex, data = hm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ sex, data = hm.oligos))
survfit(Surv(os_time, os_status) ~ sex, data = hm.oligos )

# summary(coxph(Surv(os_time, os_status) ~ surgery, data = hm.oligos ), conf.int=0.9) ## only resections

summary(coxph(Surv(os_time, os_status) ~ radiotherapy, data = hm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ radiotherapy, data = hm.oligos))
survfit(Surv(os_time, os_status) ~ radiotherapy, data = hm.oligos )

summary(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = hm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = hm.oligos))
survfit(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = hm.oligos )

summary(coxph(Surv(os_time, os_status) ~ grade, data = hm.oligos ), conf.int=0.9) 
cox.zph(coxph(Surv(os_time, os_status) ~ grade, data = hm.oligos))
survfit(Surv(os_time, os_status) ~ grade, data = hm.oligos )

# Non-hypermutant tumors
nhm.oligos <- all.oligos %>% 
  filter(HM_group == "NHM")

summary(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = nhm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ age_at_diagnosis, data = nhm.oligos))
survfit(Surv(os_time, os_status) ~ age_at_diagnosis, data = nhm.oligos )

summary(coxph(Surv(os_time, os_status) ~ sex, data = nhm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ sex, data = nhm.oligos))
survfit(Surv(os_time, os_status) ~ sex, data = nhm.oligos )

summary(coxph(Surv(os_time, os_status) ~ surgery, data = nhm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ surgery, data = nhm.oligos)) ## PH assumption violated
survfit(Surv(os_time, os_status) ~ surgery, data = nhm.oligos )

summary(coxph(Surv(os_time, os_status) ~ radiotherapy, data = nhm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ radiotherapy, data = nhm.oligos))
survfit(Surv(os_time, os_status) ~ radiotherapy, data = nhm.oligos )

summary(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = nhm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = nhm.oligos))
survfit(Surv(os_time, os_status) ~ alkylating_chemotherapy, data = nhm.oligos )

summary(coxph(Surv(os_time, os_status) ~ grade, data = nhm.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ grade, data = nhm.oligos))
survfit(Surv(os_time, os_status) ~ grade, data = nhm.oligos )
