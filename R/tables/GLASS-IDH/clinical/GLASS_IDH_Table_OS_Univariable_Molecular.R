#################################################################################################################
# Content: Univariable overall survival Cox PH models of molecular variables
# Author: Mircea Tesileanu
# Date: 2025.07.03
# R-version: 4.3.2
# Tables: Extended Data Table 1
#################################################################################################################
...
# Univariable overall survival analysis of patients with IDH-mutant astrocytomas
...
# summary(coxph(Surv(os_time, os_status) ~ PIK3R1, data = all.astros ), conf.int=0.9) ## only wildtype
...
summary(coxph(Surv(os_time, os_status) ~ PTEN, data = all.astros ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ PTEN, data = all.astros))
survfit(Surv(os_time, os_status) ~ PTEN, data = all.astros)
...
cox.zph(coxph(Surv(os_time, os_status) ~ MYCMYCN, data = all.astros))
...
# Univariable overall survival analysis of patients with IDH-mutant oligodendrogliomas
...
summary(coxph(Surv(os_time, os_status) ~ EGFR, data = all.oligos ), conf.int=0.9)
cox.zph(coxph(Surv(os_time, os_status) ~ EGFR, data = all.oligos))
survfit(Surv(os_time, os_status) ~ EGFR, data = all.oligos)

# summary(coxph(Surv(os_time, os_status) ~ PDGFRA, data = all.oligos ), conf.int=0.9)  ## only non-amp

# summary(coxph(Surv(os_time, os_status) ~ CDK46, data = all.oligos ), conf.int=0.9) ## only non-amp
