...
at the top. Two connection styles are used and **both must be configured** if you
intend to run the whole repository.

### 3.1 The `RPostgres` service (most scripts)

Scripts in `figures/R/` and `tables/R/` use:

```r
con <- dbConnect(RPostgres::Postgres(), service = "glass5reader")
```

This reads a *named service* from your PostgreSQL service file, so no credentials
appear in the code. Create `~/.pg_service.conf` with mode `600`:
...
DBI::dbGetQuery(con, "SELECT count(*) FROM clinical.cases")
DBI::dbDisconnect(con)
```

### 3.2 The ODBC DSN `glass5` (a few analysis scripts and both pipeline drivers)

`analysis/R/cnsig_expl.R`, `analysis/R/WGD_sequenza_script.R` and the `--dsn`
default of both shell pipelines use an ODBC DSN instead:
...
UserName    = <your read-only user>
Password    = <your password>
```

and make sure the driver is registered in `~/.odbcinst.ini` (or the system-wide
file):
...
The DSN name is case-sensitive in some scripts (`"GLASS5"` appears once,
`"glass5"` elsewhere); define **both** spellings as aliases if you hit a
"data source name not found" error.

### 3.3 Without database access

The underlying data is public. As noted in `README.md`, the GLASS release is on
