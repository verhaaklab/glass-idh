/*
GLASS-I: CNA Data Selection
Author: T.C. Chowdhury
Date: 2026-05-27
*/
WITH
selected_tumor_pairs AS
(
	SELECT ss.tumor_pair_barcode, ss.tumor_barcode_a, ss.tumor_barcode_b, ss.case_barcode
	FROM analysis.gold_set ss
),
selected_genes AS
(
	SELECT dr.gene_symbol,chrom,pos,cnv_direction,pathway
	FROM ref.driver_genes_codel dr
...
(
	...
	LEFT JOIN analysis.gatk_cnv_by_gene_codel c1 ON c1.aliquot_barcode = sgs.tumor_barcode_a AND c1.gene_symbol = sgs.gene_symbol
	LEFT JOIN analysis.gatk_cnv_by_gene_codel c2 ON c2.aliquot_barcode = sgs.tumor_barcode_b AND c2.gene_symbol = sgs.gene_symbol
...
WHERE st.idh_codel_subtype NOT LIKE 'IDHwt'