# GLASS-I: Mutation Burden Non-Hypermutants
...
gg_coverage_adj_mut_freq_combine_all_violin <- ggplot(
  cnv_summary_filter_primary,
  aes(x = sample_type, y = coverage_adj_mut_freq, fill = sample_type)
) +
  ...
  labs(title = "", x = "", y = "Mutation burden (Mut/Mb)") +
  theme(axis.text.x = element_text(angle = 90)) +
  ylim(0, 11.5)

gg_coverage_adj_mut_freq_combine_all_violin

gg_coverage_adj_mut_freq_combine_driver_violin <- ggplot(
  cnv_summary_filter_primary,
  aes(x = sample_type, y = coverage_adj_mut_freq, fill = sample_type)
) +
  geom_violin(trim = FALSE) +
...
gg_coverage_adj_mut_freq_combine_driver_violin
...
paired_df <- cnv_summary_filter_primary %>%
  select(case_barcode, platform, idh_codel_subtype, driver, sample_type, coverage_adj_mut_freq) %>%
  pivot_wider(
    names_from = sample_type,
    values_from = coverage_adj_mut_freq
  ) %>%
...
ggsave("~/coverage_adj_mut_freq_combine_all_violin.pdf", plot = gg_coverage_adj_mut_freq_combine_all_violin, width = 14, height = 10, units = "in", dpi = 300)
ggsave("~/coverage_adj_mut_freq_combine_driver_violin.pdf", plot = gg_coverage_adj_mut_freq_combine_driver_violin, width = 20, height = 10, units = "in", dpi = 300)